<template>
    <!--<div>
      <h1>DownsamplingBoundary details</h1>
      ...
      <table border="1">
        ...
        <tbody>
          <tr v-for="key in sampleRate" :key="key">
            ...
            <td>{{ details[key].downsampled_length }}</td>
            <td>{{ details[key].original_length }}</td>
          </tr>
        </tbody>
      </table>
    </div>-->
    ...
    <el-container>
      ...
      <el-container class="main-content">
        ...
        <div class = 'main-content-wrapper'>
          ...
          <p><strong>FileName: </strong>{{ fileName }}</p>
          <p> <strong>Method: </strong>DownsamplingBoundary</p>
          <p><strong>Complexity: </strong>{{ complexity }}</p>
        </div>
        <!--polygon image-->
        ...
        <el-table :data="sampleRate.map(key => ({
          ...
          downsampledLength: details[key]?.downsampled_length ?? '-',
          originalLength: details[key]?.original_length ?? '-'
          }))" border>
          ...
          <el-table-column prop="downsampledLength" label="Downsampled Length"></el-table-column>
          <el-table-column prop="originalLength" label="Original Length"></el-table-column>
        </el-table>
        </div>
        <!--downsampling image-->
        <el-divider/>
  ...
  export default {
    data(){
      return{
        ...
        imageUrl:null, // downsampling image url
      }
    },
